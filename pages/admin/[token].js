   ...
   console.log(data);
    ...
    return(
      <div  id='page' className='w-full h-screen bg-lime-400 flex-col flex items-center justify-center'>
        ...
        <button className=" m-10 hove button hover:underline"  onClick={handleGeneratePdf}>
		              		Generate as PDF
		    </button>
        <div ref={reportTemplateRef}>
          <div>Name: {data.data[0].name}</div>
          <div>Token: {token}</div>
          <div>{data.data[0].contact.map((i,index)=>{
          if (i[1] == token)
          return(
            <div key={index} className="text">Contact: {i[0]}  </div>
          );
          })} </div>
        </div>
